...
#' @keywords utility
create.model.frame <-
function(xmat,scale.formula,meta.data,shape.formula=NULL)
#
#  create.model.frame - creates a model frame for fitting; it includes some pre-specified and computed variables
#  and then others included in model specified by user (formula)
#
#  Arguments:
#
#  xmat     - dataframe
#  scale.formula  - user specified scale formula
#  shape.formula  - user specified shape formula
#  meta.data- user specified meta.data
#
#  Value:
#    
#  data frame
...
{
#
#  Create data frame with variables used in the formula and object # (Object id); this code
#  fix: jll 18-Nov-04; a different approach was used to allow for use of as.factor(x) etc in formula
#
    ...
    if(any(! varlist %in% names(xmat)))
       stop("The following variables used in the formula are not in the data: ",varlist[!varlist%in%names(xmat)])
    data = cbind(object = xmat$object, xmat[,varlist,drop=FALSE])
...
#
#  Include other fields such as detected, observer, binned, and optionally distance (unless null)
#
      if(is.null(data$distance))
          data=cbind(data,distance=xmat$distance,binned=xmat$binned)
      else
          data=cbind(data,detected=xmat$detected,binned=xmat$binned,observer=xmat$observer)
      if(!is.null(xmat$observer))data$observer=xmat$observer
      if(!is.null(xmat$detected))data$detected=xmat$detected
#
#  Also include # times the object was detected if it is in data
#
      if(!is.null(xmat$timesdetected))
          data=cbind(data,timesdetected=xmat$timesdetected)
#
#  If the distance data were binned, include begin and end point fields
#
      if(meta.data$binned)data=cbind(data,distbegin=xmat$distbegin,distend=xmat$distend)
#
#  If the integration width varies (int.begin/int.end are in data frame) also include those fields
#
      if(!is.null(xmat$int.begin)&!is.null(xmat$int.end))data=cbind(data,int.begin=xmat$int.begin,int.end=xmat$int.end)
#
#  If data frame contains an offset for glm also include it.
#
      if(!is.null(xmat$offsetvalue))data=cbind(data,offsetvalue=xmat$offsetvalue)
#
