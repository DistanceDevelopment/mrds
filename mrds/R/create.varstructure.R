...
#' @keywords utility
create.varstructure <-
function(model,region,sample,obs)
#
#  create.varstructure - creates samples and obs dataframes used to compute abundance and its variance
#  based on a structure of geographic regions and samples within each region.  The intent is to generalize
#  this routine to work with other sampling structures.
#
#  Arguments:
#
#  model   - fitted ddf object
#  region  - table of regions
...
#
    if(dim(samples)[1]!=dim(unique(data.frame(region=samples$Region.Label,sample=samples$Sample.Label)))[1])
       stop("Sample labels must be unique within region")
#
#   Merge again but don't use all.y which ignores samples not used
#   Change - jll 19-Jan-05 ordered samples by region and sample labels
#  
...
#
return(list(samples=samples,obs=data,region=region))
}
