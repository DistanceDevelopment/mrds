gof.rem.fi <- function(model,breaks=NULL,nc=NULL){
  # gof.rem.fi - Computes chi-square gof test for rem.fi models
  #
  # Arguments:
  ...
  #
  #  Functions used: predict(predict.rem.fi)
  ...
  return(list(chi1=list(observed=observed.count.1,
              ...
              pooled.chi=list(chisq=chisq.1+chisq.2,
                              ...
                              p=1-pchisq(chisq.1+chisq.2,2*nc-length(model$par)-1))))
}
