...
#  method    - rem or rem.fi (if rem this is being called as part of fitting) 
#
#  Functions used: assign.default.values, process.data, create.model.frame
#                  glm, predict(predict.rem.fi), NCovered (NCovered.rem.fi)
#
#  Return value: model object of class "rem.fi"
#
...
#
   result=list(call=call,data=data,model=model,
                 meta.data=meta.data,control=control,method="rem.fi")
   class(result)=c("rem.fi","ddf")
#
#
#  Create formula and model frame (if not GAM); xmat2 contains data for observer 2
#  and xmat for observer 1.  This is only done to allow a field for personnel which are
#  rotated in the observer roles of 1/2.
#
