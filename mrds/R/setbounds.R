...
#
# fix jll 18-Nov-04; added any
# jll 18-Sept-2006 moved this code back to this location from detfct.fit.opt
#
...
# fix jll 18-Nov-04; added any
	if(any(is.na(lowerbounds))){
		lowerbounds <- apply(matrix(c(initialvalues - .5*abs(initialvalues),initialvalues - 1),byrow=FALSE,ncol=2),1,min)
# fix jll 17-Aug-05 constrain power parameter in hazard rate to be >=1
		...
		if(length(lowerbounds)!=length(initialvalues))
			stop(paste("Error: incorrect number of values for lowerbounds given =",length(lowerbounds),"need ",
							length(initialvalues),"\n"))
	}
	if(any(is.na(upperbounds))){
		upperbounds <- apply(matrix(c(initialvalues + .5*abs(initialvalues),initialvalues + 1),byrow=FALSE,ncol=2),1,max)
	}else{
		if(length(upperbounds)!=length(initialvalues)){
			stop(paste("Error: incorrect number of values for upperbounds given =",length(upperbounds),"need ",
							length(initialvalues),"\n"))
		}
	}
# Bundle up our parameters to send off to detfct.fit
# jll 18-Sept-2006 Added setlower and setupper to bounds so routine will set these
# now to differentiate between user bounds and those set by program for looping on the
# optimization
#
	return(list(lower = lowerbounds, upper = upperbounds, setlower=setlower, setupper=setupper))
}
