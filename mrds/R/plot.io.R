...
#' 
plot.io <- function(x,which=c(1,2,3,4,7,8),byvar="",breaks=NULL,nc=NULL,winht=4,winwd=6,jitter.v=rep(0,3),showpoints=TRUE,showlines=TRUE,pl.col=c('black','blue','green','red'),bw.col=c(grey(0),grey(0.4),grey(0.8),grey(0.6)),black.white=FALSE,pl.den=rep(20,4),pl.ang=rep(-45,4),...){
# plot.io - Provides plots of fitted functions for an io object
#
# Arguments:
# ddf.obj               - fitted model from ddf
...
if(is.null(nc))
    ...
    hascov <- TRUE
#
#   Set up default break points unless specified or data were binned in analysis
#
...
#Taken from old plot.io function - scales det probs for ds model according to mr component
if(any(show[c(3,4,5,6)])){
#   xmat<-model$mr$data
    xmat<-model$mr$mr$data
    xmat$offsetvalue<-0
...
#New check to ensure that data is truncated if specified in the analysis. JRBB 13/08/09

    trunc.dat<-which(model$mr$data$distance>width)
    if(length(trunc.dat)>0){
      model$mr$data<-model$mr$data[-c(trunc.dat),]
    }
    xmat.p0<-model$mr$data
...
if(show[3]){
  if (.Device=="windows") win.graph()
#Uses code from plot.uncond and plot.io
  obs<-1
  #gxvalues<-p1
  gxvalues <- p1.a/delta
...
#  plot(hist.obj,freq=FALSE,main="",ylab="Detection probability",xlab="Distance",xlim = c(0, model$meta.data$width),ylim=ylim,fill=TRUE,angle=-45,density=20,col=byval1)
  ...
  if(showpoints){
    jitter.p<-rnorm(length(gxvalues[det.detected]),1,jitter.v[1])
    points(selmat$distance[det.detected],gxvalues[det.detected]*jitter.p,col=byval1,...)
  }
...
#  plot(hist.obj,freq=FALSE,main="",ylab="Detection probability",xlab="Distance",xlim = c(0, model$meta.data$width),ylim=ylim,fill=TRUE,angle=-45,density=20,col=byval1)
  ...
  #  points(selmat$distance[det.detected],gxvalues)
  if(showpoints){
  jitter.p<-rnorm(length(gxvalues[det.detected]),1,jitter.v[2])
  points(selmat$distance[det.detected],gxvalues[det.detected]*jitter.p,col=byval2,...)
  }
        title(paste("\nObserver",obs, " detections"),cex.main=0.8)
}
...
#  xmat<data
  xmat.mr<-model$mr$mr$data
  obs<-4
#  gxvalues<-p3
  gxvalues<-p1.a * p2.a/delta
#  nc<-nclass
  finebr<-seq(0,max(breaks),length=50)
  keep=dat[,vname]>=lower & dat[,vname]<=upper
...
#  xmat$offsetvalue<-0
  obs<-3
#  gxvalues<-p.
  gxvalues <- p0 * detfct.pooled.values
#  nc<-nclass
  finebr<-seq(0,max(breaks),length=50)
  keep=dat[,vname]>=lower & dat[,vname]<=upper
...
#    points(selmat$distance[det.detected],gxvalues)
#    xmat[xmat$observer==1 $ xmat$detected == 1]
  if(showpoints){
  xmat.mr<-model$mr$mr$data

  #Seen by obs1 only:
  obs1.ind<-xmat$object[which(xmat$observer==1 & xmat$detected==1 & xmat.mr$timesdetected==1)]
  obs1.ind<-sort(match(selmat$object,obs1.ind,nomatch=0),index.return=TRUE)
  obs1.ind<-obs1.ind$ix[obs1.ind$x>0]

  #Seen by obs2 only:
  obs2.ind<-xmat$object[which(xmat$observer==2 & xmat$detected==1 & xmat.mr$timesdetected==1)]
  obs2.ind<-sort(match(selmat$object,obs2.ind,nomatch=0),index.return=TRUE)
  obs2.ind<-obs2.ind$ix[obs2.ind$x>0]

  #Seen by both obs:
  obsb.ind<-xmat$object[which(xmat$observer==1 & xmat$detected==1 & xmat.mr$timesdetected==2)]
  obsb.ind<-sort(match(selmat$object,obsb.ind,nomatch=0),index.return=TRUE)
  obsb.ind<-obsb.ind$ix[obsb.ind$x>0]
