...
#
# gof.trial.fi - Computes chi-square gof test for trial.fi models
#
# Arguments: 
#
...
#
    expected.1=rep(0,nc)
    for (j in 1:nc)
       expected.1[j]=sum(predict(model,integrate=TRUE,compute=TRUE,int.range=breaks[j+1])$fitted/model$fitted)
    n=expected.1[nc]
...
#
	distances=model$data$distance[model$data$observer==1&model$data$object %in% as.numeric(names(model$fitted))]
	observed.count.1=table(cut(distances,breaks))
