...
# Set adjustment function options
	if(!is.null(modelvalues$adj.series))
		ddfobj$adjustment=list(series=modelvalues$adj.series, order=modelvalues$adj.order,scale =modelvalues$adj.scale)
	else
		ddfobj$adjustment=NULL
	if(ddfobj$type=="unif"&is.null(ddfobj$adjustment))stop("Cannot use uniform key without adjustments")
