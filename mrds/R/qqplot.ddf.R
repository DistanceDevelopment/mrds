...
# dlm	18-Aug-2005	Cleaned up control structure ambiguities
  ...
  if("ds" %in% class(model))
  ...
  else if("io" %in% class(model) | "trial" %in% class(model) | "rem" %in% class(model) )
    ...
    for(i in 1:n){
      ...
      cdfvalues[i]=predict.io.fi(model,newdata=newdata,integrate=TRUE,int.range=newdata$distance[1])$fitted
      if(model$meta.data$left!=0){
        widthvalue=predict.io.fi(model,newdata=newdata,integrate=TRUE,int.range=model$meta.data$width)$fitted
    ...
    for(i in 1:n){
      newdata=data[data$object %in% as.numeric(names(model$fitted))[i],]
      cdfvalues[i]=predict.trial.fi(model,newdata=newdata,integrate=TRUE,int.range=newdata$distance[1])$fitted

      if(model$meta.data$left!=0){
        widthvalue=predict.trial.fi(model,newdata=newdata,integrate=TRUE,int.range=model$meta.data$width)$fitted
        cdfvalues[i]=cdfvalues[i]/widthvalue
      }
     }
	 if(model$meta.data$left==0)
		 cdfvalues=cdfvalues/predict.trial.fi(model,newdata=data,integrate=TRUE)$fitted
	 ...
	 cdfvalues=sort(cdfvalues)
		...
		for(i in 1:n){
			newdata=data[data$object %in% as.numeric(names(model$fitted))[i],]
			cdfvalues[i]=predict.rem.fi(model,newdata=newdata,integrate=TRUE,int.range=newdata$distance[1])$fitted
			
			if(model$meta.data$left!=0){
				widthvalue=predict.rem.fi(model,newdata=newdata,integrate=TRUE,int.range=model$meta.data$width)$fitted
				cdfvalues[i]=cdfvalues[i]/widthvalue
			}
		}
		if(model$meta.data$left==0)
			cdfvalues=cdfvalues/predict.rem.fi(model,newdata=data,integrate=TRUE)$fitted
  ...
  plot(upper.edf,cdfvalues,xlab="Empirical cdf",ylab="Fitted cdf",xlim=c(0,1),ylim=c(0,1),...)
  abline(0,1,...)
  Dn=max(max(abs(lower.edf-cdfvalues)),max(abs(upper.edf-cdfvalues)))
  W=1/(12*n) + sum((cdfvalues - ((1:n)-.5)/n)^2)
  return(list(edf=cbind(lower.edf,upper.edf),cdf=cdfvalues,ks=list(Dn=Dn,p=pks(Dn,n)),CvM=list(W=W,p=1-pcramer(W))) )
}
