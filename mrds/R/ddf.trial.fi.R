...
# ddf.trial.fi
#
# Fits double-observer data with trial configuration and full independence (L_omega only).
#
# Arguments:
#
...
#  method    - trial or trial.fi (if trial this is being called as part of fitting) 
#
#  Functions used: assign.default.values, process.data, create.model.frame
#                  glm, predict(predict.trial.fi), NCovered (NCovered.trial.fi)
#
#  Return value: model object of class "trial.fi"
#
...
#
#  Added test to make sure that observer not used in mrmodel; jll 6/6/05
#
...
#
   GAM=FALSE
   if(modelvalues$fct=="gam") GAM=TRUE
#
...
#
    result$fitted=predict(result,newdata=xmat[xmat$observer==1&xmat$detected==1,],integrate=TRUE)$fitted
#
#   If this is not TI mode, compute the lnlU2 portion of the likelihood using the
...
#
    if(method=="trial.fi")
	{
		...
		if(meta.data$point)
			result$lnl<- result$lnl + sum(log(predict(result,newdat=xmat[xmat$observer==1&xmat$detected==1,],integrate=FALSE)$fitted*2*distances/meta.data$width^2)) - sum(log(result$fitted))
		else
			result$lnl<- result$lnl + sum(log(predict(result,newdat=xmat[xmat$observer==1&xmat$detected==1,],integrate=FALSE)$fitted/meta.data$width)) - sum(log(result$fitted))
	}
#
