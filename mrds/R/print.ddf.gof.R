...
function(x,...){
  ...
  if(!is.na(gof$chisquare$chi1$p))
    cat(paste("\nP =",format(gof$chisquare$chi1$p,digits=5)," with ",gof$chisquare$chi1$df," degrees of freedom\n",sep=""))
  else
  ...
  if(!is.null(gof$chisquare$chi2)){
    ...
    print(chitable(gof$chisquare$chi2$observed[1,],gof$chisquare$chi2$expected[1,]))
    ...
    print(chitable(gof$chisquare$chi2$observed[2,],gof$chisquare$chi2$expected[2,]))

    if(!Trial){
      cat("Capture History 11\n")
      print(chitable(gof$chisquare$chi2$observed[3,],gof$chisquare$chi2$expected[3,]))
    }

    if(!is.na(gof$chisquare$chi2$p))
      cat(paste("\nMR total chi-square =",format(gof$chisquare$chi2$chisq,digits=5),"  P= ",format(gof$chisquare$chi2$p,digits=5)," with ",gof$chisquare$chi2$df," degrees of freedom\n",sep=""))
  ...
  if(!is.null(gof$dsgof)){
      ...
      cat("Test statistic = ",format(gof$dsgof$ks$Dn,digits=5)," P = ",format(gof$dsgof$ks$p,digits=5),"\n")
      cat("\nDistance sampling Cramer-von Mises test(unweighted)\n")
      cat("Test statistic = ",format(gof$dsgof$CvM$W,digits=5)," P = ",format(gof$dsgof$CvM$p,digits=5),"\n")
  }
