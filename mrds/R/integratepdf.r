...
#  function using gstdint with stdint=FALSE
   ...
   {
       if(nobs==1)
	      return(gstdint(int.range[1,], ddfobj=ddfobj, index=1,select=NULL,width=width,
       ...
       else
	   {
		   ...
		   for(i in 1:nintegrals){
			   ...
			   {
				   if(nrow(int.range)>1)
					   integrals[i]=gstdint(int.range[i,], ddfobj=ddfobj, index=index[i],select=NULL,width=width,
							   standardize=standardize, point=point, stdint=FALSE)
  				   else
	   					integrals[i]=gstdint(int.range, ddfobj=ddfobj, index=index[i],select=NULL,width=width,
			   					standardize=standardize, point=point, stdint=FALSE)
			   } else
				   integrals[i]=gstdint(int.range[i,], ddfobj=ddfobj, index=index[1],select=NULL,width=width,
						   standardize=standardize, point=point, stdint=FALSE)
		   }
   		}
		return(integrals)
