...
#' 
plot.trial.fi <- function(x,which=c(2,4),byvar="",breaks=NULL,nc=NULL,winht=4,winwd=6,jitter.v=rep(0,3),showpoints=TRUE,showlines=TRUE,pl.col=c('black','blue'),bw.col=c(grey(0),grey(0.4)),black.white=FALSE,pl.den=rep(20,2),pl.ang=rep(-45,2),...){
# plot.trial.fi - Provides plots of fitted functions for a trial.fi object
#
# Arguments:
...
#                             3 - observer 1 detections with MR model fit and data
#                             4 - Conditional MR detection function plot:
...
#    histline(dp2,breaks=breaks,lineonly=FALSE,ylim=c(0,1),xlab=xlab,ylab="Duplicate Proportion",fill=TRUE,angle=-45,density=20,col="black")
