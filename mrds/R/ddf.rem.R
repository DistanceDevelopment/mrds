...
# ddf.trial
#
# Fits double-observer data with removal configuration and point independence.
#
# Arguments:
#
#  mrmodel   - mr model
...
#                  ddf.ds, ddf.rem.fi, predict(predict.rem), NCovered (NCovered.rem)
#
...
#
   result=list(call=call,data=data,mrmodel=mrmodel,
                 dsmodel=dsmodel,meta.data=meta.data,control=control,method="rem")
   class(result)=c("rem","ddf")
#
#  Fit the conditional detection functions using ddf.rem.fi  
#
   result$mr=ddf.rem.fi(model=mrmodel,data,meta.data,control,call,method="rem")
#
