...
#' @author Jeff Laake
#' 
ds.function <- function(model,newdata=NULL,obs="All",conditional=FALSE,pdf=TRUE,finebr)
#
#  ds.function  - computes values of conditional and unconditional detection functions and 
...
#  pdf         - if FALSE, returns p(x) and if TRUE, returns p(x)*pi(x)/integral p(x)*pi(x)
#
