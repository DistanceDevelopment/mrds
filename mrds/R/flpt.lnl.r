...
# flpt.lnl - computes negative log-likelihood values of line or point transect binned/unbinned distances
#
...
# Compute log-likelihood for any binned data
	lnl <- rep(0,dim(x)[1])
	if(any(x$binned)){
#   Get bins and create unique set of bins/covariates and indices (int.index) in that set
	...
	if(any(int.bin<0))
		int.bin <- integratepdf(ddfobj,select=which.obs,width=width,
				int.range=uniquebins,doeachint=TRUE,standardize=FALSE,point=misc.options$point)
	if(any(int.bin<0))
	{
...
#   Compute integral from left to right
	if(ddfobj$intercept.only & samelimits)
	    int.all <- integratepdf(ddfobj,select=c(TRUE,rep(FALSE,nrow(x)-1)),width=width,
...
#   integral values < 0
	...
	while (any(int1 < 0) & i < 2)
	{
		if(ddfobj$intercept.only & samelimits)
		{
			int1 <- integratepdf(ddfobj,select=c(TRUE,rep(FALSE,nrow(ddfobj$xmat))),width=width,
					int.range=int.range,doeachint=doeachint,point=misc.options$point,
					standardize=FALSE)
		}
		else
		{
			...
			int1 <- integratepdf(ddfobj,select=!x$binned,width=width,
					int.range=intrange[!x$binned,],doeachint=doeachint,point=misc.options$point,
					standardize=FALSE)
		}
	    doeachint <- TRUE	
		i <- i + 1
