...
#
# predict.io.fi - computes fitted values for p 
#                 if model object already contains fitted values it uses those,
#                 otherwise it calculates them
# 
# arguments:
#
# object     - io.fi model object
...
#   
{
   ...
   GAM=FALSE
   if("gam" %in% class(model$mr)) GAM=TRUE
   if(!integrate)
   {
...
#
#      Name int.range is misleading because it is only a right-hand point different
#      from width;  this needs to be expanded to an interval in the future
#
       if(is.null(int.range))
          pdot.list <- pdot.dsr.integrate.logistic(width,width, model$mr$coef, newdata,integral.numeric, FALSE, models,GAM, point=point)
       else
          pdot.list <- pdot.dsr.integrate.logistic(int.range,width, model$mr$coef, newdata,integral.numeric, FALSE, models,GAM, point=point)
       if(left !=0) pdot.list$pdot <- pdot.list$pdot -
                       pdot.dsr.integrate.logistic(left, width, model$mr$coef, newdata,integral.numeric, FALSE, models,GAM, point=point)$pdot
