...
# ddf.trial
#
# Fits double-observer data with trial configuration and point independence.
#
# Arguments:
#
#  mrmodel   - mr model
...
#                  ddf.ds, ddf.trial.fi, predict(predict.trial), NCovered (NCovered.trial)
#
#  Return value: model object of class "trial"
#  
# 
...
#
   result=list(call=call,data=data,mrmodel=mrmodel,
                 dsmodel=dsmodel,meta.data=meta.data,control=control,method="trial")
   class(result)=c("trial","ddf")
#
#  Fit the conditional detection functions using ddf.trial.fi  
#
   result$mr=ddf.trial.fi(model=mrmodel,data,meta.data,control,call,method="trial")
#
