...
#' @keywords utility
dht.se <-
function(model,region.table,samples,obs,options,numRegions,estimate.table,Nhat.by.sample) 
#
#  dht.se - computes std error, cv, and log-normal confidence intervals for abundance
#
#  Arguments:
#
...
#  options        - list of options that can be set (see dht)
#  numRegtions    - number of regions
#  estimate.table - table of estimate values
#  Nhat.by.sample - abundances by sample
#
#  Value:
#
#  list with 2 elements:
#
#  estimate.table - completed with se, cv and confidence limits
...
#     1) varflag=0; Binomial variance of detection process - only applicable if survey region=covered region although
#                it will scale up but it would be a poor estimator
#     2) varflag=1; delta method, with varn based on Fewster et al (2009) estimator R2 (var(n/L))
#     3) varflag=2; Innes et al variance estimator (var(N/L), except changed to resemble the form of estimator R2 of Fewster et al (2009))
#   Exceptions to the above occur if there is only one sample in a stratum. In that case it uses Poisson 
#   approximation.
#
...
#
    if (sum(region.table$Area) == 0 | options$varflag == 0) 
    {
       ...
       vc2[is.na(vc2)]=0  # jll 11/11/04 change to set missing value to 0
...
# 26 jan 06 jll; changed to use object rather than distance; also overwrites existing n
# because that can be sum(size) rather than count
...
#
        ...
        else estimate.table$k = as.vector(tapply(samples$Effort, 
            samples$Region.Label, length))
#
#       If individual abundance being computed, calculate mean
...
#           *** Changes made by LJT 19/8/04
#           Replaced
#            stratum.data=Nhat.by.sample[Nhat.by.sample$Region.Label==region$Region.Label[i],]
#           with
#           change 1/1/05 jll changed from region to region.table
#            stratum.data = Nhat.by.sample[as.character(Nhat.by.sample$Region.Label) == 
#                as.character(region.table$Region.Label[i]), ]
#           Change jll 19-Jan-05 change should have been region.table$Region not Region.Label
#
            stratum.data = Nhat.by.sample[as.character(Nhat.by.sample$Region.Label) == 
            ...
            if (length(stratum.data$Effort.y) == 1) 
                ...
                else vc2[i] = Ni^2 * (1/Ni + vars/sbar^2)
            else if (options$varflag == 1) 
                vc2[i] = (Ni * Li)^2 * varn(stratum.data$Effort.x, 
                  stratum.data$n,type=options$ervar)/sum(stratum.data$n)^2 + Ni^2 * 
                  vars/sbar^2
            else 
               if(options$varflag==2)
                 vc2[i] = varn(stratum.data$Effort.x/(scale[i] * 
                          Li), stratum.data$Nhat/scale[i],type=options$ervar)
               else
                 vc2[i] = varn(stratum.data$Effort.x/(scale[i] * 
                          Li), stratum.data$Nhat/scale[i],type=options$ervar)
...
#     The cov between regional estimate and total estimate is simply var for regional estimate.
...
#
    if (numRegions > 1) {
        ...
        vc2 = diag(vc2)
    else vc2 = as.matrix(vc2)
    vc = vc1 + vc2
#
#   jll 11/11/04 mods made here to cope with missing values and
#   0 estimates.
#
...
#
#   Use satterthwaite approx for df and log-normal distribution for 95% intervals
#
    if (options$varflag != 0) {
#
#   Change jll 20-Jan-05; set df from replicate lines to a min of 1 which avoids divide by zero
...
#       instead of cv's because it is a sum of means for encounter rate portion of variance (df.total) 
#
        if(numRegions>1)
        {
...
#
#   intervals for varflag=0; sets df=0 (changed 11/11/04 jll) 
...
#
    ...
    return(list(estimate.table = estimate.table, vc = vc, vc1=vc1.list, vc2=vc2 ))
}
