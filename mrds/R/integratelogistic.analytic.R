...
function (x, models, beta, width) 
{
#   integratelogistic.analytic  - computes integral (analytically) over x from 0 to width of a 
#       logistic detection function; For reference see integral #526 in CRC Std Math Table 24th ed
#
# Arguments:
#
#  x     - matrix of data (same as xmat)
#  models- list of model formulae
#  beta  - parameters of logistic detection function
#  width - transect half-width
#
#
#  int of f(x) = 1/(a+b*exp(px)) - in this case a=1 and b=exp(a) where a is the part of
#  the logistic that doesn't involve distance (x).
#
#  b is the portion of the integral that doesn't include distance which is computed
#  by setting x=0
#
...
#
   int<-width/(1+b)
#
#  For those values with p !=0, the following is the integral for 0 to W(width)
#
   int[p!=0] <- width - log(1+b[p!=0]*exp(p[p!=0]*width))/p[p!=0] + log(1+b[p!=0])/p[p!=0]
return(int)
}
