...
getpar=function(ddfobj,fitting="all",index=FALSE)
{
  ...
  if(!index)
  {
  	...
  	{
		if(!is.null(ddfobj$shape))pars=c(pars,rep(NA,length(ddfobj$shape$parameters)))
    	if(!is.null(ddfobj$scale))pars=c(pars,rep(NA,length(ddfobj$scale$parameters)))
  	}
  	if(fitting=="adjust" | fitting=="all")
		  if(!is.null(ddfobj$adjustment))pars=c(pars,ddfobj$adjustment$parameters)
  	else
    	  if(!is.null(ddfobj$adjustment))pars=c(pars,rep(NA,length(ddfobj$adjustment$parameters)))
	return(pars)
  }else
  {
    indices=rep(0,3)
	if(!is.null(ddfobj$shape))indices[1]=length(ddfobj$shape$parameters)
	if(!is.null(ddfobj$scale))indices[2]=indices[1]+length(ddfobj$scale$parameters)
