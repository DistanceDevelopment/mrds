...
#' @export
#' @return list object of class "dht" \item{summary}{table of statistics
#'   summarized by region} \item{N}{table of estimates for abundance}
#'   \item{D}{table of estimates for density} \item{cormat}{correlation matrix
#'   of regional abundance/density estimates and total (if more than one
#'   region)}
#' @author Jeff Laake
...
#' @keywords utility
dht <-
function(model,region.table,sample.table, obs.table=NULL, subset=NULL, se=TRUE, bootstrap=FALSE, options=list())
#
...
#
    ...
    if (numRegions > 1)
        estimate.table = data.frame(Label = c(levels(unique(samples$Region.Label)),
            "Total"), Estimate = rep(0, numRegions + 1), se = rep(NA,
            numRegions + 1), cv = rep(NA, numRegions + 1), lcl = rep(NA,
            numRegions + 1), ucl = rep(NA, numRegions + 1))
    else
        estimate.table = data.frame(Label = c("Total"), Estimate = rep(0,
        1), se = rep(NA, 1), cv = rep(NA, 1), lcl = rep(NA, 1),
        ucl = rep(NA, 1))
    if (numRegions > 1)
...
#
    ...
    if (numRegions > 1)
       var.er=c(var.er,varn(Nhat.by.sample$Effort.x,Nhat.by.sample$n,type=options$ervar))
#
#  jll 11_11_04; change to set missing values for nobs to 0 - regions with no sightings
#
    ...
    if(group)
    {
      summary.table$k = tapply(Nhat.by.sample$Sample.Label,Nhat.by.sample$Region.Label,length)  
    ...
    if (numRegions > 1)
        summary.table = data.frame(Region = c(levels(summary.table$Region),
            "Total"), rbind(summary.table[, -1], apply(summary.table[,
            -1], 2, sum)))
...
#
    if (se)
    {
        ...
        result = list(bysample=bysample.table, summary = summary.table, N = result$estimate.table,
            D = D.estimate.table, average.p=average.p, cormat = cormat,
            vc=list(total=result$vc,detection=result$vc1,er=result$vc2),
            Nhat.by.sample=Nhat.by.sample)
    }
    else result = list(bysample=bysample.table,summary = summary.table,
...
#
# Code additions by jll 18-Nov-04; the following allows for a subset statement to be added to create
...
#
    ...
    if(is.null(obs.table))
    {
        ...
        if("Sample.Label" %in% names(data) & "Region.Label" %in% names(data))
        {
           ...
           stop("Must specify obs.table because Sample.Label and/or Region.Label fields not contained in ",model$data)
     }

#*** begin Changes by LJT 19_8_04; additional changes by JLL 11_11_04 to make
#    sure all levels of factors are represented
#
...
#
    ...
    sample.table$Region.Label <- factor(sample.table$Region.Label,levels=levels(region.table$Region.Label))
    obs.table$Region.Label <- factor(obs.table$Region.Label,levels=levels(region.table$Region.Label))
    sample.table$Sample.Label <- factor(sample.table$Sample.Label)
    obs.table$Sample.Label <- factor(obs.table$Sample.Label,levels=levels(sample.table$Sample.Label))
#*** end Changes by LJT 19_8_04 and JLL 11_11_04
#
#  Assign default values to options
#
    options = assign.default.values(options, pdelta = 0.001, varflag = 2, convert.units = 1, ervar="R2")
#
#   Convert width value
#
    width= model$meta.data$width * options$convert.units
...
#
#   jll 18-Nov-04 code added to handle subset feature when labels are also in data
#
...
#   an expected S table otherwise just tables for individuals in an unclustered popn
#
    if(!is.null(obs$size))
    {
...
#
#      This computes the se(E(s)).  It essentially uses 3.37 from Ads but in place
#      of using 3.25, 3.34 and 3.38, it uses 3.27,3.35 and an equivalent cov
#      replacement term for 3.38.  This uses line to line variability whereas
#      the other formula measure the variance of E(s) within the lines and it
#      goes to zero as p approaches 1.
#
       if(se & options$varflag!=1)
       {
          if(options$varflag==2)
          {
             ...
             {
               c.stratum.data = clusters$Nhat.by.sample[as.character(clusters$Nhat.by.sample$Region.Label) ==
                  as.character(region.table$Region[i]), ]
               i.stratum.data = individuals$Nhat.by.sample[as.character(individuals$Nhat.by.sample$Region.Label) ==
                  as.character(region.table$Region[i]), ]
               Li = sum(c.stratum.data$Effort.x)
### JLL 28-Sept-09                            
               cov.Nc.Ncs[i] = covn(c.stratum.data$Effort.x/(scale[i] *
                   Li), c.stratum.data$Nhat/scale[i],i.stratum.data$Nhat/scale[i],options$ervar)
### JLL 28-Sept-09
             }
          }
          else
          ...
          cov.Nc.Ncs=cov.Nc.Ncs+diag(t(clusters$vc$detection$partial)%*%
                     solvecov(model$hessian)$inv%*%individuals$vc$detection$partial)
          ...
          Expected.S=data.frame(Region=clusters$N$Label,Expected.S=as.vector(Expected.S),
       ...
       result=list(clusters=clusters, individuals=individuals, Expected.S=as.vector(Expected.S))
    }else
    {
