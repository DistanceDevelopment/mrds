...
#
# @return list with p-values for the two tests (\code{ks.p}, \code{cramer.p})
# an dtest statistics (\code{Dn} for K-S and \code{W} for C-vM).
...
gof.pvalues <- function(model, nboot=500, ks=FALSE, progress=FALSE){
  ...
  model_W <- 1/(12*model_edf_cdf$n) + sum((model_edf_cdf$cdfvalues -
              ((1:model_edf_cdf$n)-.5)/model_edf_cdf$n)^2)
