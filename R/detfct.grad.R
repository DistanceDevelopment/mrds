...
#'    \item{adj.grd.YY}
#'  }
detfct.grad <- function(distance, par.index, ddfobj, standardize, 
                        width = NULL, left = 0) {
  ...
  ## standardize == TRUE
  if (standardize) { # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< standardize == TRUE
    ...
    if (par.index > k) {
      ...
      adj.term.0 <- switch(adj.series,
                           poly = adj.poly(zeros, scaling, 
                                           adj.order[adj.par.index] - 1),
                           herm = adj.herm(zeros, scaling, 
                                           adj.order[adj.par.index] - 1),
                           cos  = adj.cos(zeros, scaling, 
                                          adj.order[adj.par.index] - 1))
