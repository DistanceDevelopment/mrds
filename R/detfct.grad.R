...
#' gradient of detection at that distance w.r.t. the parameters. In
#' addition to the simple model above, we may specify . The adjustments are 
#' either cosine, Hermite or simple polynomial.

#' \code{detfct.grad} function which calls the others and assembles the final result
...
detfct.grad <- function(distance, ddfobj, select=NULL, index=NULL, width=NULL,
                   standardize=TRUE, stdint=FALSE, left=0){
