...
int.nonnormpdf.grad <- function(par.index, ddfobj, int.range, width, 
  ...
  if (!standardize) {
    out <- integrate(nonnormpdf.grad, lower = int.range[1], upper = int.range[2],
  ...
  #                  par.index = par.index, ddfobj = ddfobj, width = width,
  #                  standardize = FALSE)$value 
  #   # Note: C is the same regardless of standardization.. is that correct?
  #   
